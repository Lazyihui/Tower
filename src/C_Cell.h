...
void C_Cell_Init(Ctx* ctx) {
    // 盒子初始化
    for (int i = 0; i < 6; i++) {
        E_Cell* cell = Factory_CreateCell(ctx, BLUE, gridToPos(-7, 18 - i * 7),
                                          Vector2_New(std_towerCell, std_towerCell), ctx->cellIDRecord);
        // 一定要找一个东西把数据存下来
    ...
    for (int i = 0; i < ctx->cellCount; i++) {
        ...
        if (cell->cellToTower) {
            ...
            if (ctx->bltSpawnTimer <= 0) {
                E_Bullet* blt = Factory_CreatBullet(PINK, Vector2_New(1, 0), cell->pos, 5, 40);
                blt->isActive = true;
